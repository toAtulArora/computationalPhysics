program svdUSE
  ...
  real :: A(N,N),B(N),V(N,N),SIGMA(N,N),temp(N)
  integer :: i,j,errorParam
  do i=1,N
     B(i)=0
     do j=1,N
        A(i,j)=1.0/(i+j-1.0)
  ...
  call  SVD(N,N,A,V,SIGMA,N,N,temp,errorParam)
  if(errorParam==0)then
     write (*,*) "Success! Except that I don't know how to solve the equation yet :("
  end if
  write(*,*) "Fine, here's the result"
  !Note: A becomes U after evaluation
  call SVDEVL(N,N,A,V,SIGMA,N,N,B,temp,1.0E-5)
